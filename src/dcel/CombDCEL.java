...
public class CombDCEL {
	...
	public static PackDCEL redchain_by_edge(PackDCEL pdcel,
		...
		// reset 'eutil's to identify forbidden edges (include bdry edges) 
		for (int e=1;e<=pdcel.edgeCount;e++) {
			HalfEdge edge=pdcel.edges[e];
			edge.myRedEdge=null; // toss old red edge pointers
			edge.eutil=0;
		...
		// set 'eutil' -1 for forbidden edges; 'vutil' -1 for 
		...
		while (hit && pdcel.redChain!=null && !redisDone) {
			...
			for (int N=0;(N<=redN && !redisDone);N++) {
				...
				if (!doneV[v] && !(doneV[v]=isVertDone(currRed.myEdge))) {
					...
					if (pdcel.vertices[v].vutil==0) { // interior
						pdcel.vertices[v].vutil=1; // enclosable 
						int[] flower=pdcel.vertices[v].getFlower();
						for (int j=0;j<flower.length;j++) {
							// mark those next to interior
							if (pdcel.vertices[flower[j]].vutil==-1)
								pdcel.vertices[flower[j]].vutil=1; 
						}
					}
					...
					// If 'vutil'>=0, see if we can enclose it. Check 
					//    fan of edge outside current segment to see is 
					//    any are red or forbidden. If not, can close up.
					// Otherwise, following code tries to add one cclw face.
					boolean canclose=false;
					RedHEdge redge=null;
					HalfEdge spktrace=upspoke;
					if (pdcel.vertices[v].vutil>=0) {
...
//						}
					...
					//    end has 'vutil'>0, add one cclw face about v.
					else if (isMyEdge(pdcel.redChain,upspoke)==null &&
								isMyEdge(pdcel.redChain,upspoke.next)==null &&
								(upspoke.origin.vutil>0  || upspoke.twin.origin.vutil>0)) {
...
// System.out.println(rvert.vertIndx+"  --> ");			
		...
		for (int v=1;v<=vertcount;v++) {
			...
			if (vt.vutil==0 && vt.redFlag && vt.bdryFlag==0) {
				vt.vertIndx=++vcount;
				newVertices.add(vt);
...
*/
	...
	public static int zipEdge(PackDCEL pdcel,Vertex vert) {
		...
		if (redout.nextRed.myEdge.twin.origin==redin.myEdge.origin)
			throw new CombException("pasting would leave loop");
		
		// this bdry component is single slit: close up and adjust red chain 
		if (redin.myEdge.origin==redout.nextRed.myEdge.origin) {
			vert.redFlag=false;
			redin.myEdge.origin.redFlag=false;
			
			// red chain just these two? then this will be a sphere.
			...
			// else, the bdry component may be a bubble in the redchain
			if (redout.prevRed!=redin && redout.nextRed!=redin) {
				// identify
				outedge.twin=inedge;
				inedge.twin=outedge;
		...
		xvert.halfedge=null; // indicate abandon vertex

		// identify
		outedge.twin.edgeIndx=0;
		inedge.twin.edgeIndx=0;
		outedge.twin=inedge;
		inedge.twin=outedge;
	   ...
	   * that dcel's are clones, so error should leave origina data
	  ...
	  public static PackDCEL d_adjoin(PackDCEL pdc1,PackDCEL pdc2,int v1,int v2, int n) {
		  ...
		  // their ideal faces
		  dcel.Face f1=he1.twin.face; 
		  dcel.Face f2=he2.twin.face;
		  int n1=f1.getNum();
		  int n2=f2.getNum();
		  ...
		  if (n<=0) { // paste full bdrys
			  ...
			  else if (pdc1==pdc2 && f1==f2)
				  throw new CombException("'n'<=0 improper for self-pasting same bdry coomponent");
			  else
				  n=n1;
		  }
		  if (!((n1>n && n2>n) || (n1==n2 && n1<=n) ||
				  (n1==n && n2>(n+1)) || (n2==n && n1>(n+1)))) {
			  throw new CombException("edge count setting are improper");
		  }
		  
		  // for debugging, reindex 'pdc2' can use old index in 'vutil'
		  //    so we can connecct with data upon return 
		  if (pdc2!=pdc1) {
			  for (int v=1;v<=pdc2.vertCount;v++) {
		  ...
		  if (pdc1==pdc2 && f1==f2) {
			  try {
				  ...
				  HalfEdge[] fedges=new HalfEdge[n1];
				  ...
				  if ((indx2+1)==2*n) {
					  HalfEdge nxtedge=fedges[n]; // start at other ennd
					  for (int j=1;j<=n;j++) {
						  CombDCEL.zipEdge(pdc1,nxtedge.origin);
						  nxtedge=nxtedge.next;
					  }
		  ...
		  // fix pointers for pasted edges he1/2
		  ...
		  // fix h1/2 as twins (abandon their previous twins)
		  he2.twin=he1;
		  he1.twin=he2;
	  ...
	  /**
	   * Add a new vertex to some or all vertices bounding an ideal
	   * face, start with 'v' and go cclw about the boundary component
	   * (clw about the ideal face) until getting to 'w'. If w is the
	   * upstream neighbor of v, then do full bdry. If bdry comp has
	   * just two edges (i.e., it's a slit), then just close up with 'zipEdge'.
	   * Error if v and w are not on the same bdry component. Calling
	   * routine handles reprocessing of combinatorics; if full bdry is
	   * done or if there are red chain complications, then 'redChain'
	   * is set to null.
	   * 0 means error.
	   * @param pdcel PackDCEL
	   * @param v int
	   ...
	   */
	  public static int addIdeal(PackDCEL pdcel,int v,int w) {
	  
		  int count=0;
		  HalfEdge vedge=pdcel.vertices[v].halfedge.twin;
		  HalfEdge firstedge=pdcel.vertices[w].halfedge.twin.next;
		  Face idealf=firstedge.face; 
	  
		  // not a bdry edge or not on save component as v?
		  if (idealf.faceIndx>0 || idealf!=vedge.face)
			  return 0;
	  
		  // if bdry comp has just two edge, just make them twins
		  if (firstedge.next.next==firstedge) {
			  return zipEdge(pdcel,pdcel.vertices[v]);
		  }
		  
		  // essentially closes up, do whole bdry
		  if (vedge.next.next==firstedge) {
			  vedge=firstedge.prev;
		  }
		  ...
		  newv.halfedge=newout;
		  
		  // link in spike from he.origin to newv
		  newout.twin=newin;
		  he.prev.next=newin;
		  newin.prev=he.prev;
		  he.prev.next=newin;
		  newout.next=he;
		  he.prev=newout;
		  newv.halfedge.next=newout;
		  newout.prev=newv.halfedge;
		  count++;
		  
		  // Only two new reds, one fixed, the other moves; 
		  //   but if there are twinned reds coming in between
		  //   v and w, we need to redo the redchain after exit.
		  RedHEdge firstred=null;
		  RedHEdge movered=null;
		  boolean redoRed=false; // too complicated? redChain 
		  if (he.twin.myRedEdge.nextRed.twinRed!=null) {
			  redoRed=true;
		  }
		  if (!redoRed) {
			  firstred=new RedHEdge(newout);
			  firstred.nextRed=he.twin.myRedEdge.nextRed;
			  he.twin.myRedEdge.nextRed.prevRed=firstred;
			  movered=new RedHEdge(newin);
			  firstred.prevRed=movered;
			  movered.nextRed=firstred;
			  movered.prevRed=he.twin.myRedEdge;
			  he.twin.myRedEdge.nextRed=movered;
			  newout.myRedEdge=firstred;
			  newin.myRedEdge=movered;
		  }

		  boolean wholebdry=false;
		  while (fis.hasNext() && !wholebdry) {
			  HalfEdge oldhe=he;
			  he=fis.next();
			  if (he.twin.myRedEdge.nextRed.twinRed!=null) {
				  redoRed=true;
			  ...
			  HalfEdge oldin=newin;
			  HalfEdge oldout=newout;
			  newin=new HalfEdge(he.origin);
			  newout=newin.twin=new HalfEdge(newv);
			  ...
			  if (!redoRed) {
				  newin.myRedEdge=movered;
				  oldin.myRedEdge=null;
				  movered.prevRed=he.twin.myRedEdge;
				  he.twin.myRedEdge.nextRed=movered;
				  oldhe.myRedEdge=null;
			  }
		  }
	  ...
	  public static PackDCEL wrapAdjoin(PackDCEL pdc1,PackDCEL pdc2) {
		  ...
		  // vertices first; 'vutil' still holds original index
...
//			  int lastv=pdcel.vertices[v2].halfedge.twin.next.twin.origin.vertIndx;
	  ...
	  public static int baryBox_raw(PackDCEL pdcel,int v1,int v2) {
			...
			// edge; later we'll add barycenters
			...
			int vtick = pdcel.vertCount;
			ArrayList<dcel.Face> newFaces = new ArrayList<dcel.Face>();
			...
			HalfEdge twin_next=base_twin; // set for iteration
			
			dcel.Face outFace = base_twin.face;
			Vertex lastV = base_twin.origin;
			Vertex nextV = base_twin.next.origin;
			
			// relevant red edges: note that prev or
			//    next red may not be bdry -- one or 
			//    both may have a twinRed
			RedHEdge base_red = base_twin.twin.myRedEdge;
			RedHEdge next_red=base_red.nextRed; 
			RedHEdge prev_red=null; // needed in iteration
			if (pdcel.redChain == base_red)
				pdcel.redChain = next_red;
			...
			HalfEdge up_right = new HalfEdge(lastV);
			up_right.face = outFace;
			v_right.halfedge = up_right;
			
			up_right.twin = down_right;
			down_right.twin = up_right;
			
			up_right.prev=twin_prev;
			twin_prev.next=up_right;
			...
			Vertex firstV = lastV;
			RedHEdge firstRed = new RedHEdge(down_right);
			down_right.myRedEdge = firstRed;
			//   Note: red chain is broken temporarily
			firstRed.nextRed = next_red;
			next_red.prevRed = firstRed;

			// ======= iterate until finishing with v1 =======
			RedHEdge old_red = null; // need in iteration
			do {
				...
				if (pdcel.redChain==base_red)
					pdcel.redChain=prev_red;
				lastV=nextV;
				nextV=base_red.myEdge.origin;

				// new "square" face; identify with new vertex index
				vtick++; // new vertex index (created below)
				dcel.Face new_face = new dcel.Face(vtick); 
				base_twin.face = new_face;
				new_face.edge = base_twin;
				newFaces.add(new_face);

				down_left = down_right;
				up_left = up_right;
				Vertex v_left = v_right;
				...
				RedHEdge red_right = new RedHEdge(up_right);
				up_right.myRedEdge = red_right;
				...
				RedHEdge red_top = new RedHEdge(top);
				top.myRedEdge = red_top;

				// relink things
				base_twin.next.prev = down_right;
				down_right.next = base_twin.next;
				...
				// link the red's
				prev_red.nextRed = red_right;
				red_right.prevRed = prev_red;
				
				red_right.nextRed = red_top;
				red_top.prevRed = red_right;
				
				// left reds typically switchback, so remove
				if (up_left.myRedEdge!=null && 
						down_left.myRedEdge.nextRed==up_left.myRedEdge ) {

					red_top.nextRed=up_left.myRedEdge.nextRed;
					up_left.myRedEdge.nextRed.prevRed=red_top;
				...
				// else just make lastV interior
				//    (next_red is interior edge with twinRed)
				else if (up_left.myRedEdge!=null) {
					lastV.bdryFlag=0;
					lastV.redFlag=true;
				}
				// else, 'up_left.myRedEdge'==null; this is first new face
				else {
					red_top.nextRed = firstRed;
					firstRed.prevRed = red_top;
				} 

				base_red.myEdge.myRedEdge = null; // garbage old 'red'
			...
			if (closeup) {
				...
				if (pdcel.redChain == base_red)
					pdcel.redChain = firstRed.prevRed;
				Vertex newVert1 = firstRed.myEdge.origin;
				HalfEdge top = new HalfEdge(newVert1);
				HalfEdge top_tw = new HalfEdge(v_right);
				...
				// relink final 'top' and top_tw
				base_twin.next.next = top;
				top.prev = base_twin.next;
				...
				if (prev_red==firstRed) {
					red_top.prevRed=firstRed.prevRed;
					firstRed.myEdge.myRedEdge=null;
					firstV.redFlag=false;
				}
				else {
					RedHEdge lastRed=new RedHEdge(firstRed.myEdge.twin);
					firstRed.myEdge.twin.myRedEdge=lastRed;
					lastRed.prevRed=prev_red;
					prev_red.nextRed=lastRed;
					lastRed.nextRed=red_top;
					red_top.prevRed=lastRed;
				}
				
				if (next_red==up_left.myRedEdge) {
					red_top.nextRed=up_left.myRedEdge.nextRed;
					up_left.myRedEdge.nextRed.prevRed=red_top;
					up_left.myRedEdge=null;
					lastV.redFlag=false;
					...
					RedHEdge leftRed=new RedHEdge(down_left);
					down_left.myRedEdge=leftRed;
					red_top.nextRed=leftRed;
					leftRed.prevRed=red_top;
					leftRed.nextRed=next_red;
					next_red.prevRed=leftRed;
				}
			...
			int count = vtick - pdcel.vertCount;
			count += pdcel.addBaryCents_raw(newFaces);
			pdcel.vertCount += count;
			return count;
	  }
