 ...
 * Tilings may be stored in files, the packing being generated
 * on being read, may be generated by, e.g., 'pave', or may be
 * built in hierarchical fashion from subdivision rules. Much of
 * this code is devoted to this latter scenario.
 * 
 ...
 * vertex, 'vert[0]'.
 * 
 * A major concern involves a family of tilings generated by some
 ...
 * See 'baryize'.
 * 
 * (3). (default) Third is a further barycentri refinement of the
 * barycentric subdivision from (2), mode=3. Each face is further 
 * subdivided as a hex flower. This parallels the structure 
 * introduced with "dessins" and is realized in 'canonicalPack'
 ...
 * 
 * (4) Finally, there may be various refinements of any of these. 
 * Its tiles are displayed using 'canonicalPack' information; the 
 * link between these can be corrupted --- e.g., by swapping indices.
 * 
 ...
 * packing. In particular, 'mode' should be set initially, it can't 
 * really be reset without regenerating the packing from scratch. 
 * 
 * We create and save 'canonicalPack'; if there is no tiling, then a 
 * packing with TileData can be loaded or a tiling 
 * generated using 'pave'. '|ct| export -c {p}' will export 
 * 'canonicalPack' to pack 'p', '|ct| get_canon' will replace 
 * parent 'packData' with a copy of 'canonicalPack'.
 * 
 ...
 * and generate associated dual, quad, and grey/white tilings.
 * 
...
public class ConformalTiling extends PackExtender {
	...
	//    packings for tile type j. Thus vec(0) (created on reading 
	//    in a rules file) is a packing (barycentrically subdivided, 
	//    mode 3) for a single tile of type j; vec(d), if it exists 
	//    for d>0, is a packing for a tile of type j subdivided to 
	//    level d. During 'build2Depth' recursion, copy from 
...
//			}
		...
		// =========== test new code ==========
			...
			if (subdivRules==null) 
				throw new ParserException(
						"'load_rules' for subdivision rules first");
			...
			packData.tileData=SubdivisionRules.
					getRulesTD(subdivRules.tileRules.get(tt),
...
//			packData.tileData=canonicalPack.tileData.copyBareBones();
		...
		// ============= split ==========
		// TODO: I don't recall what this was used for
			...
			if (ans<=0)
				return 0;
			canonicalPack=buildMode3(packData.tileData);
			// new packData should remain unchanged
			return packData.nodeCount; 
		}
