...
public class D_ProjStruct extends PackExtender {
	...
	public int cmdParser(String cmd, Vector<Vector<String>> flagSegs) {
		...
		// ======== weak_rif ===========
			...
			count = D_ProjStruct.vertRiffle(packData, aspects,2,PASSES,vlink);
			if (count < 0) {
			...
			count = D_ProjStruct.vertRiffle(packData, aspects,1,PASSES,vlink);
			if (count < 0) {
		...
		// ========== ccode ===========
			...
			for (int v=1;v<=packData.nodeCount;v++) {
				...
				while (sis.hasNext()) {
					...
					if (w>v && !he.isBdry()) 
						edata.add(D_ProjStruct.logEdgeTs(packData,
								he,aspects));
				}
			}
...
//		p.setRadius(v,factor*p.getRadius(v));
	...
	public static int propogate(TriAspect[] asps,int f,
		...
		if (mode==3) { // schwarzian with radii
			...
			CircleSimple sC = 
				D_Schwarzian.getThirdCircle(s,(J+2)%3, bm_f,hes);
		...
		if (mode==4) { // schwarzian with labels
			...
			// compute the target circle
			double s=asps[g].schwarzian[(J+2)%3];
			CircleSimple sC = D_Schwarzian.getThirdCircle(s,(J+2)%3, bm_f,hes);
